...
onready var enfriamiento_power_up_vuelo = $EnfriamientoPowerUpVuelo
...
func tomar_direccion():
	var direccion = 0
	if puede_moverse:
		direccion = Input.get_action_strength("mov_dcha") - Input.get_action_strength("mov_izq")
		if direccion == 0:
			animacion.play("idle")
		else:
			if direccion < 0:
...
func volar():
	enfriamiento_power_up_vuelo.start()
	acel_caida = 64
...
func caida_al_vacio():
	if position.y > camara.limit_bottom / 1.7:
		respawn()
...
func _on_EnfriamientoPowerUpVuelo_timeout():
